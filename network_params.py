...
connection_matrix = np.zeros((net_dict['nr_microcircuits'], net_dict['nr_microcircuits'], len(net_dict['populations']), len(net_dict['populations'])))

for i in range(net_dict['nr_microcircuits']):
    for j in range(net_dict['nr_microcircuits']):
        if i == j:
            connection_matrix[i,j] = conn_probs['self']
        elif ((i+1)%net_dict['nr_microcircuits']) == j or ((i-1)%net_dict['nr_microcircuits']) == j:
            connection_matrix[i,j] = conn_probs['adj']
        else:
            connection_matrix[i,j] = conn_probs['oth']
