...
class Network:
    ...
    def create_and_connect(self):
        """ Creates and connects all network nodes.
        ...
        for i in range(self.net_dict['nr_microcircuits']):
            self.__create_neuronal_populations()

        for i, mc in enumerate(self.mcs):
            if len(self.sim_dict['rec_dev']) > 0:
                self.__create_recording_devices(mc)
            if self.net_dict['poisson_input']:
                self.__create_poisson_bg_input(mc)
        # Only the one microcircuit gets thalamic input
            if i == round((self.net_dict["nr_microcircuits"]-0.5)/2) and self.stim_dict['thalamic_input']:
                self.__create_thalamic_stim_input(mc)

        for i, mc_i in enumerate(self.mcs):
            for j, mc_j in enumerate(self.mcs):
                self.__connect_neuronal_populations(i, j)
        nest.Prepare()
        nest.Cleanup()
...
#                 firing_rates_interval[0], firing_rates_interval[1])
    ...
    def __create_recording_devices(self, mc):
        ...
        if 'spike_recorder' in self.sim_dict['rec_dev']:
            if nest.Rank() == 0:
                print('  Creating and connecting spike recorders.')
            sd_dict = {'record_to': 'ascii',
                       'label': os.path.join(self.data_path, 'spike_recorder')}
            self.spike_recorders.append(nest.Create('spike_recorder',
                                               n=self.num_pops,
                                               params=sd_dict))

        for i, target_pop in enumerate(mc):
            if 'spike_recorder' in self.sim_dict['rec_dev']:
                bla = nest.Connect(target_pop, self.spike_recorders[-1][i])
    ...
    def __create_thalamic_stim_input(self, mc):
        ...
        self.thalamic_population.append(nest.Create(
            'parrot_neuron', n=self.stim_dict['num_th_neurons']))

        self.poisson_th.append(nest.Create('poisson_generator'))
        self.poisson_th[-1].set(
            rate=self.stim_dict['th_rate'],
            start=self.stim_dict['th_start'],
            stop=(self.stim_dict['th_start'] + self.stim_dict['th_duration']))

        nest.Connect(self.poisson_th[-1], self.thalamic_population[-1])
        ...
        for i, target_pop in enumerate(mc):
            ...
            nest.Connect(
                self.thalamic_population[-1], target_pop,
                conn_spec=conn_dict_th, syn_spec=syn_dict_th)

    def __connect_neuronal_populations(self, mc_i, mc_j):
        """ Creates the recurrent connections between neuronal populations. """
#         if nest.Rank() == 0:
#             print('Connecting neuronal populations recurrently between microcircuit ' + str(mc_i) + ' and microcircuit ' + str(mc_j) + ' with connectivity ' + str(self.num_synapses[mc_i][mc_j]))

        for i, target_pop in enumerate(self.mcs[mc_i]):
            for j, source_pop in enumerate(self.mcs[mc_j]):
                if self.num_synapses[mc_i][mc_j][i][j] >= 0.:
                    conn_dict_rec = {
                        'rule': 'fixed_total_number',
                        'N': self.num_synapses[mc_i][mc_j][i][j]}
    ...
    def __create_poisson_bg_input(self, mc):
        ...
        self.poisson_generators.append(poisson_generator)
        ...
        for i, target_pop in enumerate(mc):
            ...
            nest.Connect(
                self.poisson_generators[-1][i], target_pop,
